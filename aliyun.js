...
const refresh_token = process.env['ali_refresh_token'];   // 需要的环境变量
...
!(async () => {
    ...
    await main();
    // 去掉最后的换行
    out_log = out_log.substring(0, out_log.length - 1)
    await wechat_send(out_log);
...
async function sign() {
    try {
        ...
        let data = JSON.parse(a.body);
        if (data.success) {
            print(`已连续签到${data.result.signInCount}天!`);
...
async function sign_in_reward(day) {
    try {
        ...
        let data = JSON.parse(a.body);
        if (data.success) {
            print(`奖励:${data.result.name},${data.result.description},${data.result.notice}!`);
        } else {
            print(`奖励获取失败:${data.message}!`);
        }
...
async function wechat_send(text) {
    if (!QYWX_KEY) return;
    ...
    let myRequest = {
        url: url, method: method, headers: headers, body: JSON.stringify(body)
    };
    let a = await $.http.post(myRequest);
    console.log(a.body);
}
